...
class SearchViewController: UIViewController {
    ...
    // declare array type Download contain all file download is actived
...
extension SearchViewController: UISearchBarDelegate {
  func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
    ...
    if !searchBar.text!.isEmpty {
        ...
        let expectedCharSet = NSCharacterSet.urlHostAllowed
        ...
        if let url = downloadTask.originalRequest?.url?.absoluteString {
            ...
            if let trackIndex = trackIndexForDownloadTask(downloadTask: downloadTask) {
                DispatchQueue.main.async(execute: {
                    let indexPath = NSIndexPath(row: trackIndex, section: 0)
